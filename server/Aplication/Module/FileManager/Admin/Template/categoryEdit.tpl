<form name="fileCategoryEdit" method="post" action="{$URL.POST}" class="standard">
	<fieldset>
	...
	<fieldset>
		...
		<select name="parent_id">
			...
			<option value="{$ITEM.id}" {if $oCategory->parent_id == $ITEM.id}selected="selected"{/if}>{$ITEM.name}</option>
			{/foreach}
		</select>
	...
	<div class="buttons" style="padding-left:160px;">
		<a class="btnSmall orange" href="javascript:void(0);" onClick="$('form[name=fileCategoryEdit]').submit();">{bw_i18n txt="save_changes"}</a>
		<a class="btnSmall black last" href="javascript:void(0);" onClick="$('<div />')._window({ id: 'categoryDelete', title:'{bw_i18n txt="confirmation_of_removal"}', content: '{$URL.DELETE}' });">{bw_i18n txt="delete"}</a>
	</div>
</form>